let arrUpcoming = []

function infoUpcoming (data){
    for (let event of data.events) {
        if (event.date > data.currentDate){
            arrUpcoming.push (event);
        }
    }
}

infoUpcoming(data);

let upComing = document.querySelector("#upComing");

function cardCreator (arrUpcoming, upComing){
    for (let event of arrUpcoming){
        upComing.innerHTML += `<div class="col">
        <div class="card" id=${event.id} style="width: 18rem;">
            <img src=${event.image} class="card-img-top" alt="festival">
            <div class="card-body">
                <h5 class="card-title">${event.name}</h5>
                <p class="card-text">${event.description}</p>
                <p>Price: ${event.price}</p>
                <a href="../details/details.html" class="btn btn-dark">See more</a>
            </div>
        </div>
    </div>`
    }
}

cardCreator(arrUpcoming, upComing);